...
## Diagram interpretation (FILL skills and agents)

The diagram you provided can be read as a layered system with two planes: **skills** (knowledge + procedures) at the top, and **agents** (runtime executors) at the bottom.

- **Top band (skills)**: The global skill, reasoning skill, and Nerve API skill are documents. They describe what the system *should do* and how to do it. The inGraph database sits above the reasoning skill because that skill explains how to query or interpret the knowledge graph for intent-to-workload mapping.
- **Bottom band (agents)**: The captain agent lives in the core runtime (inSwitch Core). It loads the global skill by default to understand the domain and workflows. When a task requires specialization, it creates sub-agents on demand:
  - A **reasoning agent** that loads the reasoning skill to map user intent to workloads/containers.
  - An **API agent** that loads the Nerve API skill to deploy those workloads.
- **Arrows labeled “load skills on demand”** show that skills are not preloaded everywhere; they are pulled into context only when a relevant sub-agent is created.
- **Arrows labeled “sub agents created on demand”** show that sub-agents are ephemeral resources that appear only when needed.
- The **Nerve API (REST)** box on the right emphasizes separation of concerns: only the API agent talks to the operational system.

This diagram matches the actual multi-agent flow: the captain interprets intent, delegates reasoning and API execution to separate agents, and keeps the overall context clean by restricting each sub-agent to exactly one skill.

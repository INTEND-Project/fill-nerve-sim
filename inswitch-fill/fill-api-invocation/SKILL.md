...
description: Use the simulated FILL Nerve API to list nodes/workloads, resolve workload versions, and apply DNA target deployments. Use when users need to deploy, undeploy, or inspect machine analytics workloads.
---
...
If the input is a list of workload:version entries, use "Deploy or undeploy via DNA target" section, to generate YAML payload and call "apply target". Make sure to not remove the already deployed workloads on the target machine. If the a different version of the same workload is already deployed, check with the user whether to replace it with the new version.
